...
{
    comparacoes++;
    for (int i = (h + cor); i < nLista; i += h)
    {
        Movie tmp = filmes.filmes[i];
        int j = i - h;
        
        while ((j >= 0) && (strcmp(filmes.filmes[j].original_language, tmp.original_language) > 0))
        {
            filmes.filmes[j + h] = filmes.filmes[j];
            j -= h;
            movimentacoes++;
            comparacoes++;
        }
        comparacoes++;
        filmes.filmes[j + h] = tmp;
        movimentacoes++;
        comparacoes++;
    }
}
...
int main()
{
    ...
    fp = fopen("matrícula_shellsort.txt", "w+");
