...
typedef struct
{
    Movie filmes[MAX_MOVIES];

} Lista;
Lista filmes;

void OrdenarRemocao(int pos)
{

    if (pos != nLista)
    {
        filmes.filmes[pos] = filmes.filmes[pos + 1];
        OrdenarRemocao(pos + 1);
    }
}

void OrdenarInsercao(int posInf, int posSup)
{
    if (posInf != posSup + 1)
    {
        filmes.filmes[posSup + 1] = filmes.filmes[posSup];
...
Movie RemoverFinal()
{
    Movie temp = filmes.filmes[nLista];
    nLista--;
    return temp;
}
...
Movie Remover(int pos)
{
    ...
    Movie temp = filmes.filmes[pos];
    OrdenarRemocao(pos);
    nLista--;
    return temp;
}

void InserirInicio(Movie filme)
{
    OrdenarInsercao(0, nLista);
    filmes.filmes[0] = filme;
    nLista++;
}

void InserirFinal(Movie filme)
{
    if (&filmes.filmes[0] != NULL)
    {

        nLista++;
        filmes.filmes[nLista] = filme;
    }
    else
    {
        filmes.filmes[0] = filme;
...
void Inserir(Movie filme, int pos)
{

    OrdenarInsercao(pos, nLista);
    filmes.filmes[pos] = filme;
    nLista++;
}
...
void ImprimirFilme(int pos)
{
    ...
    movie_print(&filmes.filmes[pos]);
}

int CompararSelectionSort(int troca,int c, int pos, Movie menor)
{
   
    if (pos < nLista)
    {
      
        if (filmes.filmes[pos].original_title[c] < menor.original_title[c])
        {
            menor = filmes.filmes[pos];
            troca = pos;
            c = 0;
            comparacoes++;
        }
        else if (strcmp(filmes.filmes[pos].name, menor.name) != 0 && filmes.filmes[pos].original_title[c] == menor.original_title[c])
        {
            c++;
            pos--;
            comparacoes += 2;
       ...
       troca = CompararSelectionSort(troca,c, pos+1, menor);
    }
...
void SelectionSortRecursiva(int pos)
{
    ...
    if (pos < nLista)
    {
        ...
        int troca = pos;
        int c = 0;
        Movie menor = filmes.filmes[pos];
        comparacoes ++;
        troca = CompararSelectionSort(pos,0, pos, filmes.filmes[pos]);
      
        tmp = filmes.filmes[troca];
        filmes.filmes[troca] = filmes.filmes[pos];
        filmes.filmes[pos] = tmp;
        movimentacoes++;

        SelectionSortRecursiva(pos + 1);
    }
}
...
int main()
{
    ...
    for (int i = 0; i < count_movies; i++)
    {
        ...
        filmes.filmes[i] = movies[i];
        nLista++;
    }

    SelectionSortRecursiva(0);
    for (int i = 0; i < nLista; i++)
    {
        movie_print(&filmes.filmes[i]);
    }

    FILE * fp;
    
    
    float Tresultado = clock()/(float)CLOCKS_PER_SEC;

   fp = fopen ("matrícula_selecaoRecursiva.txt", "w+");
   fprintf(fp, "727225 \t %d \t %d \t %f",comparacoes, movimentacoes,Tresultado);
