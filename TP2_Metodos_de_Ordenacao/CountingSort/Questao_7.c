...
int getMaior(){
    ...
    for(int i = 0;i<nLista;i++){
       if(filmes.filmes[i].duration > maior){
           maior = filmes.filmes[i].duration;
       }
    }
    return maior;
}
...
void CountingSort()
{
    ...
    Movie ordenado[nLista+1];
    
    for(int i = 0; i< sizeof(count)/sizeof(count[0]);count[i] = 0, i++,comparacoes++);
    
    for(int i = 0; i< nLista+1; count[filmes.filmes[i].duration]++,i++,comparacoes++);
    for(int i = 1; i< sizeof(count)/sizeof(count[0]); count[i]+= count[i-1], i++,comparacoes++);
    for(int i = nLista; i>=0;ordenado[count[filmes.filmes[i].duration]-1] = filmes.filmes[i],count[filmes.filmes[i].duration]--,i--,comparacoes++,movimentacoes++);

    for(int i = 0;i<=nLista;i++,movimentacoes++,comparacoes++){
        filmes.filmes[i] = ordenado[i];
    }
...
int main()
{
    ...
    fp = fopen("matrícula_countingsort.txt", "w+");
